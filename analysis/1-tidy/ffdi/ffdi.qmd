...
ffdi_fireregions_tidy |>
  ...
  group_by(DIST_NAME, month, decade) |>
  summarise(
...
seasonal_highdanger_days |>
  group_by(DIST_NAME, month) |>
  summarise(
    min_days = min(p_gte_90, na.rm = TRUE),
    max_days = max(p_gte_90, na.rm = TRUE)) |>
  ungroup() ->
seasonal_highdanger_ranges

ggplot(seasonal_highdanger_days) +
  aes(x = month, colour = decade) +
  # vertical arrow lines for each month
  geom_linerange(
    aes(ymin = min_days, ymax = max_days),
    data = seasonal_highdanger_ranges,
    colour = "lightgrey"
  ) +
  # geom_line() +
  geom_point(
  ...
  labs(
    x = NULL, y = NULL,
    title = "Percentage of days in top 10% of fire danger",
    subtitle = "New England (NSW)"
  )
