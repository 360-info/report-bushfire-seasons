...
calc_field_avgs <- function(raster_path, boundaries) {
  ...
  boundaries |>
    ...
    select(SAL_CODE21, slug, SAL_NAME21, STE_CODE21, avg_ffdi) |>
    st_drop_geometry() |>
    unnest(avg_ffdi) |>
    pivot_longer(-c(SAL_CODE21, slug, SAL_NAME21, STE_CODE21)) |>
...
if (!file.exists(here("data", "suburbs", "SAL_2021_AUST_GDA2020.shp"))) {
  suburbs_url <- paste0(
...
here("data", "suburbs", "SAL_2021_AUST_GDA2020.shp") |>
  read_sf() |>
  mutate(slug = janitor::make_clean_names(SAL_NAME21)) ->
suburbs
...
here("data", "copernicus-firedanger", "australia") |>
  ...
  tibble(path = _) |>
  mutate(ffdi_df = map(path, calc_field_avgs, suburbs)) ->
ffdi_fireregions
...
ffdi_fireregions_tidy |>
  ...
  group_by(SAL_CODE21, SAL_NAME21, slug) |>
  mutate(pct_90 = quantile(value, 0.9)) |>
  ungroup() |>
  group_by(SAL_CODE21, SAL_NAME21, slug, month, then_or_now) |>
  summarise(
    n_gte_90 = sum(value > pct_90, na.rm = TRUE)) |>
  ungroup() |>
  pivot_wider(
    id_cols = c(SAL_CODE21, SAL_NAME21, slug, month),
...
ggplot(filter(seasonal_highdanger_days, SAL_NAME21 == "New England")) +
  aes(x = month) +
  geom_point(
